...
def a_star(ori_start, ori_goal, row_index_start, row_index_end, target_count, target_num, current_row,
    ...
    while not frontier.empty():
        ...
        eq_count = 0
        for i in range(18):
            current_num = flattened_current_state[i]
...
def get_path(start, goal):
    ...
    print("72二分")
    path, current_state = a_star_split(start, goal, 0, 72, 27, 27)
    print("72二分结束")
    if path:
        zero_row, zero_col = next(
    ...
    print("63二分")  # 7
    path, current_state = a_star_split(start, goal, 0, 63, 27, 27)
    print("63二分结束")
    if path:
        zero_row, zero_col = next(
            (i, j) for i, row in enumerate(start) for j, val in enumerate(row) if val == blank_num)
    ...
    for i in range(start_count, start_count + 9 * 2):
        ...
        current_row = i // 9
        # path, current_state = a_star(start[current_row:], goal[current_row:], i + 1, current_row)
        path, current_state, star_result = a_star(start, goal, 6, 9,
                                                  i + 1 - start_count, i + 1,
                                                  current_row, 0)
